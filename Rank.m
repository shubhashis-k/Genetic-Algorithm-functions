...
%%Rank%%
        ...
        SelectionProb = sortrows(SelectionProb,1);
        rankedProb = zeros(PopLength,2);
        
        rankedProb(:,1) = SelectionProb(PopLength:-1:1,2);
        rankedProb(:,2) = (1:PopLength);
        
        rankedProb(:,2) = rankedProb(:,2)/10;
        
        for i = 2:1:PopLength
            rankedProb(i,2) = rankedProb(i,2) + rankedProb(i-1,2);
        end
        
        SelectionChance = max(rankedProb(:,2))*rand(PopLength,1);      
        
        for i=1:1:PopLength
            for(j=1:1:PopLength)
                if(SelectionChance(i)<rankedProb(j,2))
